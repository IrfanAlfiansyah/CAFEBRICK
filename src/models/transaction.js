...
const createNewTransaction = (body) => {
  return new Promise((resolve, reject) => {
    const { product_id, quantity, size, sub_total, shipping, tax, total, address } = body;
    const sqlQuery =
    ...
    db.query(sqlQuery, [product_id, quantity, size, sub_total, shipping, tax, total, address])
      .then(({ rows }) => {
        const response = {
          data: rows[0],
        };
...
const updateTransaction = (id, body) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(sqlQuery, [product_id, quantity, size, sub_total, shipping, tax, total, address, id])
      .then((data) => {
        const response = {
          data,
        };
